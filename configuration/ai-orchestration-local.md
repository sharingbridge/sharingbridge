...
| `.venv\Scripts\Activate.ps1` | **No** | Generated by `python -m venv`; activates that venv in PowerShell |

If `Activate.ps1` is **empty** or activation fails, delete `.venv` in that repo and run `python -m venv .venv` again. Alternative without activation:
