...
Architecture: [authentication.md](./authentication.md). Persistence: [database.md](./database.md). Mobile (initiator): [mobile-client.md](./mobile-client.md).
...
| **Test users** | **Audience** ([direct link](https://console.cloud.google.com/auth/audience)) | Under **Test users** → **+ Add users** → add every Gmail you will sign in with (coordinator + initiator). Required while **Publishing status** is **Testing**. |
...
### 0.5 Optional now — Android client (mobile initiator)
...
5. **Refresh** after a initiator registers an intent on mobile (local).
...
## Mobile (initiator) — after Phase 2
