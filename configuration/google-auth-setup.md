...
Step-by-step guide for **coordinator web** + **initiator mobile** using Google OAuth.  
...
| **Android OAuth client** | `sharingbridge-mobile-app` (initiators) |
...
5. **Test users** — [Audience](https://console.cloud.google.com/auth/audience) → **Test users** → **+ Add users** → every coordinator and initiator Gmail for dev.  
...
### 2.2 Android (initiator mobile app)
...
| Goal | Approach |
|------|----------|
| Initiator + Google + Render/local APIs | **Android emulator** or device — use **Android** OAuth client (§2.2) |
| Coordinator | **Web dashboard** — **Web** OAuth client (§2.1) |
| Initiator on Windows desktop (dev only) | `flutter run -d windows` with `--dart-define=AUTH_TOKEN=…` (dev mint on user-service) |
...
1. Sign in once with the coordinator Gmail (mobile as initiator is fine) so a row exists in **`users`**.
...
- **`user_roles` may include `donor`/`initiator` and `coordinator`** for the same user.
- **Web** sign-in mints JWT `role: coordinator` when `coordinator` is in `user_roles`, else `initiator`.
- **Mobile** sign-in mints JWT `role: initiator` (requires `donor` or `initiator` in `user_roles`; coordinators can also use the mobile app).
- Initiator-only account on **web** → limited dashboard (not `403`); account with **no** `donor`/`initiator`/`coordinator` → `403 wrong_client_role`.
...
6. **Refresh** on the dashboard shows order initiations (after an initiator registers one on mobile).

### 6.2 Initiator (mobile)
...
2. Use a Gmail with **`donor` or `initiator`** in `user_roles` (default for all users). Coordinators with both roles can use mobile as initiator and web as coordinator.
3. Complete **Help a seeker** → copy instructions to register an order intent.
4. On web (coordinator), **Refresh** — you should see that intent (with initiator `user_id` in the list).
...
| `403 wrong_client_role` on mobile | Missing `donor`/`initiator` in `user_roles` | Sign in again (`donor` is ensured on each auth); check DB `user_roles` |
...
- [ ] Web sign-in (coordinator) + mobile sign-in (initiator) tested
