...
**Overall progress vs target model:** [IMPLEMENTATION_APPROACH.md](./IMPLEMENTATION_APPROACH.md) § **Where we are**. **Per-repo APIs and next tasks:** this file (sections below).

| If you need… | Read |
|--------------|------|
| **Overall progress vs target model** | `development/IMPLEMENTATION_APPROACH.md` § **Where we are** |
...
| Eco kitchen / Connection | `order_code`, `initiation_route`, consent columns | SQL **M4** |
| FCM tokens | `device_tokens` | SQL **M5** + notification-service |
...
**Progress map (all workstreams):** [IMPLEMENTATION_APPROACH.md](./IMPLEMENTATION_APPROACH.md) § **Where we are**.
...
- Labels: `initiationLabels.ts` — **Direct order**, eco kitchen pledge/self-pay routes, **kitchen commit** copy on Actions.
- **Connection** panel (`ConnectionLookupPanel`) — order code handoff after kitchen commit.
- **Updates** banner (`DashboardNotificationsBanner`) — connection-ready rows on sign-in / Refresh (**§4d-b** in manual guide).
- Deploy: [configuration/web-client.md](../configuration/web-client.md).

### `sharingbridge-notification-service`
- Node 20: `POST /internal/connection-ready` webhook → FCM multicast via `device_tokens` table.
- Env: `DATABASE_URL`, `WEBHOOK_SECRET`, `FIREBASE_SERVICE_ACCOUNT_JSON` — [configuration/notification-service-local.md](../configuration/notification-service-local.md).
- Wired from integration-service `CONNECTION_NOTIFY_WEBHOOK_URL` when eco kitchen commits.
- Deploy: [configuration/backend-render.md](../configuration/backend-render.md) § Notification service · root `render.yaml`.

### Other repos
- `sharingbridge-ai-orchestration`: deterministic MVP (FastAPI); live LLM optional.
- `sharingbridge-photo-service`: reference photo upload (Docker on Render).
- `sharingbridge-api-gateway`, `sharingbridge-order-service`, `sharingbridge-infra`, `sharingbridge-deployment`: README / not started.
- `sharingbridge-location-safety`: **archived**.
...
| Photo / AI / notify | `sharingbridge-photo-service`, `sharingbridge-ai-orchestration`, `sharingbridge-notification-service` |
| Not in Track A MVP | `sharingbridge-api-gateway`, `sharingbridge-order-service`, `sharingbridge-location-safety` (archived) |
...
1. **Transactional email** — Resend/SendGrid in notification-service (FCM push shipped; email still notification-only copy).
