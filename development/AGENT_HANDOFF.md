...
The **donor-setup slice** that is live in code is intentionally a **minimal MVP** (“MVP‑0”): flows and APIs are wired end‑to‑end while **preferences and user-service payee data stay file‑backed JSON** (easy local and Render/Railway iteration, with the limits of ephemeral disks and single-instance file stores).
...
- File-backed preferences store (`src/preferencesStore.js`) accessed via a `PreferencesRepository` abstraction (`src/preferencesRepository.js`).
  ...
  - `UserServicePreferencesRepository` is wired to `sharingbridge-user-service` (`GET/PUT /v1/users/{user_id}/donor-presets`, **`POST …/donor-presets/delete-item`** for single-row delete) and forwards payee auth headers.
...
- **Seeker demand (Phase C.1):** `POST /v1/seeker-demands` (payee or coordinator reporter), `GET /v1/seeker-demands`, `GET /v1/demand/board` — Postgres `seeker_demands` via `PostgresSeekerDemandStore`; migration [schema-seeker-demands-migration.sql](../configuration/schema-seeker-demands-migration.sql).
