...
- 29 tests, all green via `npm test`.
...
1. **Operational hardening for token flow.**
   - Move `AUTH_TOKEN_SECRET` to managed secret storage per environment.
   - Rotate dev default secret and disallow fallback default outside local dev.
