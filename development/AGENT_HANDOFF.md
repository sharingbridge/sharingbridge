...
Legacy one-off import from old `data/preferences.json`: `npm run backfill:user-service-presets` — see [USER_SERVICE_PREFERENCES_MIGRATION.md](./USER_SERVICE_PREFERENCES_MIGRATION.md).
...
4. **Legacy JSON import (one-time only).**
   - Run `npm run backfill:user-service-presets` only if an old `data/preferences.json` exists from pre-Postgres local runs.
