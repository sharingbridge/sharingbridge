...
| Per-repo bootstrap checklists | `development/MVP_BOOTSTRAP_ISSUES.md` |
| BRD assumptions | `requirements/SharingBridge_Business_Requirement.md` |
| GitHub org/repo renames | `development/GITHUB_ORG_AND_REPO_RENAMES.md` |
...
- File-backed `UserStore` + **`POST /v1/auth/google`** (coordinator allowlist via `data/coordinators.json` / `COORDINATOR_EMAILS`).
...
3. **Track A — hosted MVP backend.** [configuration/backend-render.md](../configuration/backend-render.md). Smoke: MANUAL_TESTING_GUIDE **§6** (hosted); web dashboard **§4**. Then **Track B:** `sharingbridge-photo-service` + mobile upload.
